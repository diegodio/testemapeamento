...
def gerar_mapeamento_inicial(alunos: dict[str, str]) -> dict[Posicao, str]:
    """Distribui os alunos em filas, ordenados pelo número da chamada."""
    numeros = sorted(alunos.keys(), key=lambda n: (int(n) if n.isdigit() else 10**9, n))
    mapa: dict[Posicao, str] = {}
    for indice, numero in enumerate(numeros):
        fila = indice // ALUNOS_POR_FILA + 1
        posicao = indice % ALUNOS_POR_FILA + 1
        mapa[(fila, posicao)] = alunos[numero]
    return mapa
