...
def _carregar_info(pasta_turma: str) -> dict[str, str]:
    """Carrega info.json e devolve {nome_normalizado: nome_original}."""
    caminho = Path(pasta_turma) / "info.json"
    if not caminho.is_file():
        return {}
    with open(caminho, encoding="utf-8") as f:
        dados = json.load(f)
    return {normalizar_nome(nome): nome for nome in dados}


def localizar_imagem(pasta_turma: Path, nome_aluno: str) -> Path | None:
    """Procura {NOME COMPLETO}.{ext} na pasta da turma,
    comparando por nome normalizado, de forma case-insensitive."""
